      ...
      <div class="trade-content-wrap clearfix">
            <div class="trade-form clearfix">
                ...
                <div class="select-btn-group-wrap">
                    <button ref="inItem" v-for="(item,index) in monthtrade.InvestedFunds"  :data-value="item.val"  @click="changein(item,index)" :key="index" class="JS-trade-applyAmount select-btn">
                      {{item.text}}
                    </button>
                ...
                <div class="select-btn-group-wrap last-child"> 
                  <div v-for="(item,index) in monthtrade.Application" ref="apItem" @click="changeap(item,index)" :key="index" class="JS-trade-investAmount tehui-btn select-btn">
                    <div :data-mutiple="item.val">{{item.text}}</div>
                    <div class="monthinterest">月利率:{{item.ly_text}}</div>
                ...
                <div class="trade-res">
                    <table>
                        ...
                        <tr class="line">
                            ...
                            <td id="monthtimetip">自动延期,最长3个交易月</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td style="position: relative;" class="clearfix">
                                <span class="JS-trade-mgamt" id="fee">{{glf}}</span>&nbsp;元 <span>x 1月</span>
                        ...
                        <tr>
                            ...
                            <td><span class="JS-payall font-color-red font-bold font-size-16" id="paymoney">{{zj}}</span>&nbsp;&nbsp;元<span class="tips">（准备资金 = 投资本金 + 月管理费 x 月数） </span></td>
                        </tr>
...
export default {
  ...
  data () {
    return {
        ...
        monthtrade:{
          ...
          // 申请操盘资金列表
          Application:[
            {id:0,text:'3倍',ly_text:'2.00%',ly_num:0.02,val:3,jjxbs:0.875025,zsxbs:0.825,glfbs:0.0054},
            {id:1,text:'4倍',ly_text:'2.20%',ly_num:0.022,val:4,jjxbs:0.9,zsxbs:0.86,glfbs:0.008},
            {id:2,text:'5倍',ly_text:'2.40%',ly_num:0.024,val:5,jjxbs:0.9167,zsxbs:0.8833,glfbs:0.01},
            {id:3,text:'6倍',ly_text:'2.60%',ly_num:0.026,val:6,jjxbs:0.9285,zsxbs:0.90,glfbs:0.0132},
            {id:4,text:'7倍',ly_text:'2.80%',ly_num:0.028,val:7,jjxbs:0.9375,zsxbs:0.9125,glfbs:0.0154},
            {id:5,text:'8倍',ly_text:'3.00%',ly_num:0.03,val:8,jjxbs:0.9444,zsxbs:0.9222,glfbs:0.02},
            {id:6,text:'9倍',ly_text:'3.00%',ly_num:0.03,val:9,jjxbs:0.9500,zsxbs:0.9300,glfbs:0.0225},
            {id:7,text:'10倍',ly_text:'3.00%',ly_num:0.03,val:10,jjxbs:0.9545,zsxbs:0.9364,glfbs:0.027},
          ],
        },
        trzj:200,
        sqcpbs:3,
  ...
  computed:{
    ...
    jjx:function(){
      return Math.floor(this.monthtrade.Application[this.sqcpbs-3].jjxbs * ((this.trzj * this.sqcpbs) + this.trzj)*100)/ 100;
    },
    zsx:function(){
      return Math.floor(this.monthtrade.Application[this.sqcpbs-3].zsxbs * ((this.trzj * this.sqcpbs) + this.trzj)*100)/ 100;
    },
    glf:function(){
      return Math.floor(this.monthtrade.Application[this.sqcpbs-3].ly_num * (this.trzj*this.sqcpbs)*100)/ 100;
    },
    zj:function(){
